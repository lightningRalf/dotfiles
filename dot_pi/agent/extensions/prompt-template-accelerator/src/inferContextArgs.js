...
export async function inferContextArgs({ pi, ctx, providedArgs }) {
  ...
  const objectiveHint = readLatestObjectiveHint(ctx.sessionManager);

  const firstProvidedArg = providedArgs.find((arg) => arg.trim().length > 0);
  const roughThought = firstProvidedArg || objectiveHint || `Task in ${repoName}`;
