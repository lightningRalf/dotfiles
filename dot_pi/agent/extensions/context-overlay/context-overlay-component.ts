...
import { appKeyHint, type KeybindingsManager, type Theme } from "@mariozechner/pi-coding-agent";
...
export class ContextOverlayComponent {
	...
	private buildHeader(innerW: number): string {
		...
		const mode = this.frozen ? this.theme.fg("warning", "FROZEN") : this.theme.fg("success", "LIVE");
	...
	private buildFooter(): string {
		const close = appKeyHint(this.keybindings, "interrupt", "close");
		const toggle = appKeyHint(this.keybindings, "expandTools", "freeze/live");
